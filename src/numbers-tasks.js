...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModulus1 = Math.hypot(x1, y1);
  const vectorModulus2 = Math.hypot(x2, y2);
  return Math.acos(scalarProduct / (vectorModulus1 * vectorModulus2));
}
...
function getCountOfOddNumbers(number) {
  ...
  return number % 2 === 0
    ? getCountOfOddNumbers(numberAbs - 1)
    : 1 + getIntegerOnString(getCountOfOddNumbers(numberAbs - 1), 10);
}
